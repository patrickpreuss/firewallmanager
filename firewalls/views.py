...
class RuleCreate(CreateView):
    model=Rule
    template_name='firewalls/rule_create.html'
    success_url='/rule/list'
    fields = ['source','destination','port_number','connection_type','approved']

    @method_decorator(login_required(login_url="/login"))
    def dispatch(self, *args, **kwargs):
        return super(RuleCreate, self).dispatch(*args, **kwargs)

    def form_valid(self, form):
        src_ip = Host.objects.filter(host_name=form.instance.source)[0].ip_address
        dest_ip	= Host.objects.filter(host_name=form.instance.destination)[0].ip_address
        form.instance.source_ip =	src_ip
        form.instance.destination_ip = dest_ip
        return super(RuleCreate, self).form_valid(form)


class RuleList(ListView):
    model = Rule
    template_name='firewalls/rule_list.html'
    @method_decorator(login_required(login_url="/login"))
    def dispatch(self, *args, **kwargs):
        return super(RuleList, self).dispatch(*args, **kwargs)



class RuleUpdate(UpdateView):
    model=Rule
    template_name='firewalls/rule_update.html'
    success_url = '/rule/list'
    fields = ['source','destination','port_number','connection_type','approved']

    @method_decorator(login_required(login_url="/login"))
    def dispatch(self, *args, **kwargs):
        return super(RuleUpdate, self).dispatch(*args, **kwargs)

class RuleDelete(DeleteView):
    model=Rule
    template_name='firewalls/rule_confirm_delete.html'
    success_url='/rule/list'

    @method_decorator(login_required(login_url="/login"))
    def dispatch(self, *args, **kwargs):
        return super(RuleDelete, self).dispatch(*args, **kwargs)

## End of Rule CRUD Section ##
...
#    rpcHost=Host.objects.filter(id=request.POST.get("name"))[0].ip_address
 #   print str(rpcHost)
    ...
    if request.is_ajax():
        if request.method=='POST':
            try:
                ...
                rpcClient.connect("tcp://"+request.POST.get("name")+":4242")
                print rpcClient
